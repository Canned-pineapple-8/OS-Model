...
from UI.devices_widgets.IOColumn import IOColumn
from UI.devices_widgets.CPUColumn import CPUColumn
from UI.processes_widgets.ProcessList import ProcessListWidget
from UI.devices_widgets.MemoryViewer import MemoryViewer
...
class OSUI(QMainWindow):
    def __init__(self, os_model, update_interval_ms=100):
        ...
        logo = QLabel("Модель ОС")
        logo.setFont(QFont(MONO_FONT, 14, weight=QFont.Weight.Bold))
        top_panel.addWidget(logo)
        top_panel.addStretch()
        main_layout.addLayout(top_panel)

        # слева - CPU+IO+терминал, справа - параметры системы и процессов
        ...
        self.process_widget.setMinimumHeight(340)
        self.process_widget.setMinimumWidth(280)
        upper_split = QSplitter(Qt.Orientation.Horizontal)
        upper_split.setHandleWidth(6)

        # левая часть - таблица процессов
        self.process_widget.setMinimumWidth(220)
        upper_split.addWidget(self.process_widget)

        # правая часть - параметры процессов
        self.process_params_widget = ProcessParamsWidget(self.os_model)
        ...
        def mem_cb():
            try:
                mv = MemoryViewer(self.os_model.memory_manager)
                mv.exec()
            except Exception:
                pass
        self.sys_params = SystemParamsWidget(self.os_model, memory_btn_callback=mem_cb, parent=self)
        self.sys_params.setMinimumHeight(260)
        right_layout.addWidget(self.sys_params)

        right_panel.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)

        main_split.addWidget(left_panel)
        main_split.addWidget(right_panel)
        main_split.setSizes([620, 560])
